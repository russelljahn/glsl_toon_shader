...
vec4 noise(vec4 co){
    return vec4(
      ...
      fract(sin(dot(co.xw ,vec2(12.9898,78.233))) * 43758.5453),
      fract(sin(dot(co.yw ,vec2(12.9898,78.233))) * 43758.5453)
	);
}
...
void main() {
  ...
  vec4 diffuseThickness = (1.0-specular*LMs)*LMd;
  vec4 unlitThickness = (1.0-diffuse)*LMa;

  float texAmount = snoise(vec2(normal.z, normal.y));
  vec4 texThickness = texAmount * tex;

  gl_FragColor = clamp(diffuseThickness + unlitThickness + texThickness, 0.0, 1.0);
  gl_FragColor.a = 1.0;
