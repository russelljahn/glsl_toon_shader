...
void main() {
	...
	vec4 noiseColor = 
		vec4(
			vec3(
				cos(phaseR + speed1*timePreviousFrame) * snoise(vec2(lightDirection.x, lightDirection.z)), 
				cos(phaseG + speed1*timePreviousFrame) * snoise(vec2(lightDirection.y, lightDirection.x)), 
				cos(phaseB + speed1*timePreviousFrame) * snoise(vec2(lightDirection.z, lightDirection.y))
			),
			snoise(normal.xz)
		);
  ...
  if (diffuse < 0.75) {
	...
	gl_FragColor = 0.15*specular*LMs + 0.15*diffuse*LMd + 2.0*noiseColor;
}
