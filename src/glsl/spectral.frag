...
void main() {
	...
	vec4 noiseColor = 
		vec4(
			vec3(
				snoise(vec2(normal.x, normal.x)), 
				snoise(vec2(normal.y, normal.y)), 
				snoise(vec2(normal.z, normal.z))
			),
			snoise(normal.xz)
		);
	...
	gl_FragColor = vec4(vec3(5.0*specular*LMs + 5.0*diffuse*LMd + 0.5*noiseColor).rgb, 0.25);
}
